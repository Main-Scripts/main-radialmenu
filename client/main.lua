...
-- Player Menu
RegisterNetEvent('radialmenu:client:openPlayerMenu')
AddEventHandler('radialmenu:client:openPlayerMenu', function()
    ESX.TriggerServerCallback('radialmenu:getPlayerData', function(data)
        if not data then return end
        
        local menuItems = {}
        
        if Config.PlayerOptions.showId then
            table.insert(menuItems, {
                label = 'Show ID',
                icon = 'fas fa-id-card',
                onSelect = function()
        ...
        if Config.PlayerOptions.ragdoll then
            table.insert(menuItems, {
                label = 'Ragdoll',
                icon = 'fas fa-male',
                onSelect = function()
        ...
        if Config.PlayerOptions.surrender then
            table.insert(menuItems, {
                label = 'Hands Up',
                icon = 'fas fa-hands',
                onSelect = function()
        ...
        if Config.PlayerOptions.walkstyles then
            table.insert(menuItems, {
                label = 'Walk Styles',
                icon = 'fas fa-walking',
                onSelect = function()
        ...
        table.insert(menuItems, {
            ...
            icon = 'fas fa-search',
            onSelect = function()
                TriggerEvent('ox_inventory:openInventory', 'player')
            end
        })
        ...
        lib.showRadial('player_menu')
    end)
end)

-- Vehicle Menu
RegisterNetEvent('radialmenu:client:openVehicleMenu')
AddEventHandler('radialmenu:client:openVehicleMenu', function()
    ...
    if PlayerData.job and PlayerData.job.name == 'mechanic' then
        table.insert(menuItems, {
            ...
            onSelect = function()
                ...
                TriggerServerEvent('radialmenu:server:repairVehicle', plate)
            end
        })
    end
...
-- Job Menu
RegisterNetEvent('radialmenu:client:openJobMenu')
AddEventHandler('radialmenu:client:openJobMenu', function()
    if not PlayerData.job or not Config.JobMenus[PlayerData.job.name] then
        lib.notify({
            title = 'No Job Actions',
            description = 'No job actions available',
            type = 'error'
        })
        return
    end
    ...
    lib.showRadial('job_menu')
end)

-- Items Menu
RegisterNetEvent('radialmenu:client:openItemsMenu')
AddEventHandler('radialmenu:client:openItemsMenu', function()
    if not Config.UseOxInventory then
        lib.notify({
            title = 'Items Disabled',
            description = 'Quick items are disabled',
            type = 'error'
        })
        return
    end
    ...
    lib.showRadial('items_menu')
end)

-- Emote Menu
RegisterNetEvent('radialmenu:client:openEmoteMenu')
AddEventHandler('radialmenu:client:openEmoteMenu', function()
    local menuItems = {}
    ...
    for category, emotes in pairs(Config.Animations) do
        table.insert(menuItems, {
            ...
            onSelect = function()
                OpenEmoteCategoryMenu(category, emotes)
            end
        })
    end
    ...
    lib.showRadial('emote_menu')
end)

-- GPS Menu
RegisterNetEvent('radialmenu:client:openGPSMenu')
AddEventHandler('radialmenu:client:openGPSMenu', function()
    local menuItems = {}
...
function OpenVehicleDoorsMenu(vehicle)
    ...
    for _, doorInfo in pairs(doors) do
        table.insert(menuItems, {
            ...
            icon = 'fas fa-door-open',
            onSelect = function()
                ToggleVehicleDoor(vehicle, doorInfo.door)
            end
        })
    end
...
function OpenEmoteCategoryMenu(category, emotes)
    local menuItems = {}
    ...
    lib.showRadial('emote_category_' .. category)
end
...
-- Key Binding
RegisterKeyMapping('openradialmenu', 'Open Radial Menu', 'keyboard', Config.RadialKey)
...
function OpenPlayerTargetMenu(targetPed)
    ...
    if #menuItems > 0 then
        ...
        lib.showRadial('target_player_menu')
    end
end
